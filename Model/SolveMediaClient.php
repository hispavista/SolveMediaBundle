...
/* This code is based on code from solvemedialib,
 * https://portal.solvemedia.com/media/download/solvemedialib.php
 */

namespace Hispavista\SolveMediaBundle\Model;

/**
 * Description of SolveMediaClient
 *
 * @author rubenma
 */
class SolveMediaClient {
     ...
     */
    private function _adcopy_qsencode($data) {
        $req = "";
     ...
     */
    private function _adcopy_http_post($host, $path, $data, $port = 80) {
    ...
    function solvemedia_get_html($pubkey, $error = null, $use_ssl = false) {
        if ($pubkey == null || $pubkey == '') {
            die("To use solvemedia you must get an API key from <a href='" . SolveMediaClient::ADCOPY_SIGNUP . "'>" . SolveMediaClient::ADCOPY_SIGNUP . "</a>");
        }

        if ($use_ssl) {
            $server = SolveMediaClient::ADCOPY_API_SECURE_SERVER;
        } else {
            $server = SolveMediaClient::ADCOPY_API_SERVER;
        }
    ...
    function solvemedia_check_answer($privkey, $remoteip, $challenge, $response, $hashkey = '') {
        ...
        $response = $this->_adcopy_http_post(SolveMediaClient::ADCOPY_VERIFY_SERVER, "/papi/verify", array(
            'privatekey' => $privkey,
            'remoteip' => $remoteip,
            'challenge' => $challenge,
            'response' => $response
        ...
        if (strlen($hashkey)) {
            ...
            if ($hash != $answers[2]) {
                $adcopy_response->setIs_valid(false);
                $adcopy_response->setError('hispavista.solve_media.invalid_captcha');
                return $adcopy_response;
            }
        }

        if (trim($answers [0]) == 'true') {
            $adcopy_response->setIs_valid(true);
        } else {
            $adcopy_response->setIs_valid(false);
            $adcopy_response->setError('hispavista.solve_media.invalid_captcha');
        }
    ...
    function solvemedia_get_signup_url($domain = null, $appname = null) {
        return SolveMediaClient::ADCOPY_SIGNUP . "?" . $this->_adcopy_qsencode(array('domain' => $domain, 'app' => $appname));
    }
